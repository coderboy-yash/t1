...
import { calculateWinner, findBoardsWon } from "./calculateWinner.jsx";
import SmallBoard from "./smallboard.jsx";
import { minMaxMove } from "./AI/ai.jsx";
...
export default function BigBoard({ robot, appStatus, playerIsX }) {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    for (let j = 0; j < 3; j++) {
      ...
      if (boardsFinished[k] && boardsFinished[k] !== "draw") {
        children.push(
          <div className={styles.bigScore} key={k + "bigScore"}>
            {boardsFinished[k]}
          </div>
        );
      }
      boardArray.push(<td style={{position:"relative",border:"0.2rem solid var(--border-color)",padding:"0.4rem"}} key={k + "cell"}>{children}</td>);
    }
  ...
  return (
    <>
    <div style={{border:"2px solid var(--border-color)",borderRadius:"20px",padding:"20px",width:"80%",margin:"20px auto"}}>
      <div style={{marginLeft:"37%",marginBottom:"10px"}} className={styles.status}>{status}</div>
